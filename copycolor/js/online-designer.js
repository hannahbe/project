...
$(document).ready(function () {
    ...
    document.getElementById('designer-container').addEventListener('mousedown', function (evt) {
        ...
        var foundTxt = false;
        var j = txtArray.length - 1;
        while (j >= 0 && !foundTxt) {
            if (txtArray[j].mouseIn('item', mousePos.x, mousePos.y)) {
                $('#txt_edit').val(txtArray[j].text);
                $('#edit-text').show(250);
                txtArray[j].select['enabled'] = true;
                if (!txtArray[j].drag) {
                    //save the previous position so when finished dragging we save it to history
                    drag_text['x'] = txtArray[j].select['x'];
                    drag_text['y'] = txtArray[j].select['y'];
                }
                txtArray[j].drag = true;
                // if the text the user selected isn't on top, put it on top by moving it to the end of the array
                if (j < txtArray.length - 1) {
                    txtArray[txtArray.length - 1].select['enabled'] = false;
                    var tempTxt = txtArray[j];
                    txtArray.splice(j, 1);          //remove element from imgArray
                    txtArray.push(tempTxt);         //push element to the end of imgArray

                    //save changes to history
                    design_history.saveSuperposition('text', i);
                }
                foundTxt = true;
            }
            // if the mouse is outside the image and we are not resizing it, then disable select
            else if (!txtArray[j].enableResize(mousePos.x, mousePos.y)) {
                $('#edit-text').hide(250);
                txtArray[j].select['enabled'] = false;
                txtArray[j].drag = false;
            }
            ...
            if (txtArray[j].select['enabled'] && txtArray[j].enableResize(mousePos.x, mousePos.y)) {
                resize_text['x'] = txtArray[j].select['x'];
                resize_text['y'] = txtArray[j].select['y'];
                resize_text['w'] = txtArray[j].select['width'];
                resize_text['h'] = txtArray[j].select['height'];
                txtArray[j].drag = false;
                foundTxt = true;
            }
            j--;
            if (foundTxt && imgArray.length > 0)
        ...
        if (!foundTxt) {
            var foundImg = false;
            var i = imgArray.length - 1;
            //we loop through the images from top one to bottom one to check if one of them has to be dragged or resized
            while (i >= 0 && !foundImg) {
                // if the mouse is in an image then enable drag and select for this image
                if (imgArray[i].mouseIn('item', mousePos.x, mousePos.y)) {
                    imgArray[i].select = true;
                    if (!imgArray[i].drag) {
                        //save the previous position so when finished dragging we save it to history
                        drag_image['x'] = imgArray[i].x;
                        drag_image['y'] = imgArray[i].y;
                    }
                    imgArray[i].drag = true;

                    // if the image the user selected isn't on top, put it on top by moving it to the end of the array
                    if (i < imgArray.length - 1) {
                        imgArray[imgArray.length - 1].select = false;
                        var tempImg = imgArray[i];
                        var tempImgLoaded = imgLoaded[i];
                        imgArray.splice(i, 1);          //remove element from imgArray
                        ...
                        //save changes to history
                        design_history.saveSuperposition('image', i);

                    }
                    foundImg = true;
                }
                // if the mouse is outside the image and we are not resizing it, then disable select
                else if (!imgArray[i].enableResize(mousePos.x, mousePos.y)) {
                    imgArray[i].select = false;
                    imgArray[i].drag = false;
                }
                // if select is enabled and the mouse is in one of the resize boxes, then disable drag and enable relevant resize flag (this action is done by enableResize)
                if (imgArray[i].select && imgArray[i].enableResize(mousePos.x, mousePos.y)) {
                    resize_image['x'] = imgArray[i].x;
                    resize_image['y'] = imgArray[i].y;
                    resize_image['w'] = imgArray[i].width;
                    resize_image['h'] = imgArray[i].height;
                    imgArray[i].drag = false;
                    foundImg = true;
                }
                i--;
    ...
    document.body.addEventListener('mouseup', function (evt) {
        ...
        if (txtArray.length > 0 && txtArray[txtArray.length - 1].select['enabled']) {
            ...
            if (txtArray[txtArray.length - 1].drag) {
                ...
                txtArray[txtArray.length - 1].drag = false;
            }

            if (txtArray[txtArray.length - 1].resizing()) {
                txtArray[txtArray.length - 1].disableResize();
        ...
        //the only selected image must be the top one, so we only check if the top image is selected to disable drag and resize on mouseup
        else if (imgArray.length > 0 && imgArray[imgArray.length - 1].select) {

            if (imgArray[imgArray.length - 1].drag) {
                //save dragging to history
                design_history.saveMove('image', drag_image['x'], drag_image['y'], imgArray[imgArray.length - 1].x, imgArray[imgArray.length - 1].y);
                imgArray[imgArray.length - 1].drag = false;
            }

            if (imgArray[imgArray.length - 1].resizing()) {
                imgArray[imgArray.length - 1].disableResize();
                //save resizing to history
    ...
    online_designer.addEventListener('mousemove', function (evt) {
        ...
        //we check again on the top image to see if we are resizing it or dragging it, change cursor look and call the relevant function
        if (txtArray.length > 0 && txtArray[txtArray.length - 1].select['enabled']) {
            //change cursor look
            if (txtArray[txtArray.length - 1].mouseIn('right_box', mousePos.x, mousePos.y) || txtArray[txtArray.length - 1].mouseIn('left_box', mousePos.x, mousePos.y))
                $('html,body').css('cursor', 'ew-resize');
            else if (txtArray[txtArray.length - 1].drag)
                $('html,body').css('cursor', 'move');
            else if (!txtArray[txtArray.length - 1].resizing())
                $('html,body').css('cursor', 'default');

            //call relevant function
            if (txtArray[txtArray.length - 1].resizing())
                txtArray[txtArray.length - 1].resizeTextBox(mousePos.x, mousePos.y);
            else if (txtArray[txtArray.length - 1].drag)
                txtArray[txtArray.length - 1].move(mousePos.x, mousePos.y);
        }

        //we check again on the top image to see if we are resizing it or dragging it, change cursor look and call the relevant function
        else if (imgArray.length > 0 && imgArray[imgArray.length - 1].select) {
            //change cursor look
            if (imgArray[imgArray.length - 1].mouseIn('top_box', mousePos.x, mousePos.y) || imgArray[imgArray.length - 1].mouseIn('bottom_box', mousePos.x, mousePos.y))
                $('html,body').css('cursor', 'ns-resize');
            else if (imgArray[imgArray.length - 1].mouseIn('right_box', mousePos.x, mousePos.y) || imgArray[imgArray.length - 1].mouseIn('left_box', mousePos.x, mousePos.y))
                $('html,body').css('cursor', 'ew-resize');
            else if (imgArray[imgArray.length - 1].drag)
                $('html,body').css('cursor', 'move');
            else if (!imgArray[imgArray.length - 1].resizing())
                $('html,body').css('cursor', 'default');

            //call relevant function
            if (imgArray[imgArray.length - 1].resizing())
                imgArray[imgArray.length - 1].resizeImage(mousePos.x, mousePos.y);
            else if (imgArray[imgArray.length - 1].drag)
                imgArray[imgArray.length - 1].move(mousePos.x, mousePos.y);
        }
    ...
    document.body.addEventListener('keyup', function (evt) {
        ...
        //left arrow => rotate selected item left, right arrow => rotate selected item right
            ...
            if (txtArray.length > 0 && txtArray[txtArray.length - 1].select['enabled']) {
                var rotateItem = txtArray[txtArray.length - 1];
                design_history.saveRotate('text', code == LEFT_ARROW ? 'left' : 'right');     //save rotation to history
                change = true;
            }
            else if (imgArray.length > 0 && imgArray[imgArray.length - 1].select) {
                var rotateItem = imgArray[imgArray.length - 1];
                design_history.saveRotate('image', code == LEFT_ARROW ? 'left' : 'right');     //save rotation to history
                change = true;
            }
            if (change)
                code == LEFT_ARROW ? rotateItem.rotate -= ROTATION : rotateItem.rotate += ROTATION;
        }
